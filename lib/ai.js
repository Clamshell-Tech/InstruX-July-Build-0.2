...
export async function safeGroqJsonCall(messages, maxTokens = 7000, temperature = 0.7) {
  let attempts = 0;
  const maxAttempts = 3;
  let lastError = null;
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      // Sanitizer: strip markdown code blocks and conversational text
      ...
      lastError = e;
      attempts++;
      console.warn(`Groq JSON generation attempt ${attempts} failed, retrying in 2s...`, e.message);
